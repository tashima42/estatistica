...
O sono está intimamente ligado ao bem-estar pscicológico, segundo a Academia Brasileira do Sono (2024), o sono é fundamental para a regulação emocional, a capacidade cognitiva, prevenção de transtornos mentais, resiliência psicológica, dentre outros aspectos da saúde mental.

Este relatório utiliza dados de uma pesquisa realizada pelo setor de recursos humanos (RH) com uma amostra dos colaboradores de uma empresa. Seu objetivo é sugerir mudanças práticas e realistas de comportamento, principalmente ligados ao sono, que promovam a saúde mental dos colaboradores. Para isso, como contexto, serão apresentadas análises dos dados coletados e ao final, recomendações para o setor de RH.
...
Colaboradores com distúrbios do sono tem uma mediana de horas de sono, muito menor do que colaboradores sem nenhum distúrbio. Quando distúrbios são comparados, colaboradores com insônia tem uma mediana de sono menor que os com apneia do sono, porém os dois cenários são preocupantes, porque o quartil inferior dos dois distúrbios estão em níveis muito baixos (gráfico 4).
```{R echo=FALSE}
...
Com base nas análises realizadas, é recomendado ao setor de recursos humanos realizar ações, principalmente de conscientização, direcionadas aos principais problemas observados, utilizando como principal fonte a cartilha da Academia Brasileira do Sono (2025).
